...
- [ ] **(Remaining ~half-day) Drive the last 53 `tsc` errors to zero.** These are genuine per-site fixes: 14 property-access (`TS2339`), 9 type-assignment (mostly wrong theme tokens like `WeeklyTrajectoryCard` color literals), 7+6 navigation param-type errors, 5 `onPrimary`/`sm` token typos (`TS2551`), 5 null-safety (`TS18047`, e.g. `HealthAutoSync` `newSettings`). One is a real runtime bug, not just noise: `src/firebase/firebase.ts:4` imports `getReactNativePersistence`, which Firebase v12 no longer exports, so **native auth persistence is broken and users are logged out on every app restart** — fix during Phase 2/3.
