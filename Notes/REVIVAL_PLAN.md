...
- [ ] **(DEVICE, you) Bump deps and retest — the cheapest high-probability fix.** Native Hermes `instanceof` segfaults are frequently engine/SDK bugs fixed in point releases. Concrete versions checked 2026-07-01:
  - `firebase` 12.8.0 → **12.15.0** (minor within v12; Firestore does heavy internal `instanceof` on async paths, so this is the most directly relevant bump). `npm i firebase@12.15.0`.
  - Run **`npx expo install --fix`** — the project has drifted from SDK 54's recommended versions (`expo` 54.0.31→54.0.35, `expo-notifications` 0.29→0.32, `expo-updates`, `expo-font`, `expo-image-picker`, `babel-preset-expo`, `jest-expo`). Aligning these pulls the Hermes/RN patches Expo ships for SDK 54.
  - I did NOT apply these in the tsc-cleanup session because they cannot be verified without an EAS build + device, and applying them blind risked destabilizing the now-clean-compiling tree. Apply them at the start of the Phase 2 build so they are verified together. After applying, re-run `npx tsc --noEmit` and `npm test` (the `expo-notifications` 0.29→0.32 jump may touch the trigger/permission types fixed in Phase 1).
